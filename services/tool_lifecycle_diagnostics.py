...
# ============================================================
# سؤال تكلفة عن مشروع -- كشف حتمي (2026-08-15)
# ============================================================
# تلات مرات آدم نفى وجود أرقام موجودة، وفي المرة التالتة **ماناداش ولا
# أداة واحدة** (`selected_tools: []`) -- رد من ذاكرة المحادثة وقال بنفسه
# "لسه نفس الإجابة"، يعني كان بيقلّد نفيه القديم المسجّل في التاريخ.
#
# ده بيخلي كل الإصلاحات اللي قبل كده بلا أثر: مخرج الأداة اتظبط، ووصفها
# اتوضّح، وقاعدة التوجيه اتكتبت في البرومبت -- وكلهم بيفترضوا إن الأداة
# هتتنادى. لما الموديل بيرد من غير أي نداء، مفيش حاجة فيهم بتشتغل.
#
# القاعدة في البرومبت اقتراح، والفرض هنا حتمي -- نفس آلية
# `detect_explicit_tool_request` اللي اتعملت لحادثة 2026-07-27.

def normalize_ar(text):
    """يوحّد الاختلافات الإملائية اللي مالهاش أي فرق في المعنى.

    الفئة دي كسرت الفحوصات هنا **أربع مرات**، وكل مرة على حرف واحد:

        "مش موجودة عندي"  ضد  "مش موجود عندي"     تاء مربوطة
        "فى مقايسة"       ضد  "في مقايسة"          ألف مقصورة
        "اسعار"           ضد  "أسعار"              همزة
        "تأسيس"           ضد  "تاسيس"              همزة على ألف

    وأحمد بيكتب الشكلين في نفس المحادثة. مطاردة الصيغ واحدة واحدة لعبة
    الخسارة فيها مؤكدة -- التوحيد بيقفل الفئة كلها مرة واحدة.
    """
    if not text:
        return ""
    text = str(text)
...
# الكلمات مكتوبة **بعد** التوحيد (مفيش همز ولا تاء مربوطة ولا ألف مقصورة)
_COST_WORDS = (
    "تكلف", "كلف", "سعر", "اسعار", "بكام", "كام", "قيم",
    "اجمالي", "ميزاني", "جنيه", "فلوس",
)
# "بتاع" لوحدها بتغطي بتاع/بتاعة (اللي بتتوحّد لـ"بتاعه"). كان فيه
# "بتاعت" هنا وهي غلط -- التوحيد بيدّي ه مش ت، فالمدخل ده كان ميت.
_PROJECT_WORDS = ("مشروع", "عميل", "عند ", "بتاع")

# مفردات بتخص المقايسة لوحدها -- وجودها كفاية من غير كلمة مشروع، لأن
# "مقايسة" و"حصر" مالهمش معنى برة سياق مشروع أصلاً.
_ESTIMATE_WORDS = ("مقايس", "حصر كميات", "بنود")


def detect_project_cost_question(user_message: str) -> bool:
    """السؤال ده عن تكلفة حاجة في مشروع؟

    كشف حتمي وواسع بالقصد: تكلفة الخطأ غير متناظرة. لو فرضنا القراءة
    والسؤال مكانش عن مشروع، الثمن جولة أداة زيادة. لو ما فرضناهاش والسؤال
    كان عن مشروع، الثمن رقم كاذب بيوصل أحمد -- وده حصل تلات مرات.
    """
    text = normalize_ar(user_message)
    if not text:
        return False
    if any(w in text for w in _ESTIMATE_WORDS):
        return True
    return (any(w in text for w in _COST_WORDS)
            and any(w in text for w in _PROJECT_WORDS))
...
def guard_against_unread_no_data_claims(reply_text: str, chat_id=None) -> str:
    ...
    # نفس الكاشف الحتمي اللي الحلقة بتفرض بيه -- مصدر واحد للحقيقة
    # بدل قايمتين بتفرقوا مع الوقت، وبتوحيد إملائي يقفل فئة
    # الاختلافات اللي كسرت الفحص أربع مرات.
    cost_question = detect_project_cost_question(question)
    read_estimate = (not no_turn_context) and _ESTIMATE_TOOL in tools

    # ⚠️ الشرط مابيقراش صياغة النفي بالقصد (2026-08-15).
    #
    # النسخة القديمة كانت بتشترط تطابق نمط نفي في الرد، وفشلت **تلات
    # مرات** على الصياغة:
    #   1. "مفيهوش أي بيانات"      -- الكلمة مكانتش في القايمة
    #   2. رد من غير مفردات مقايسة  -- الموضوع كان بيتقرا من الرد
    #   3. "مش موجودة عندي"        -- تاء مربوطة زيادة عن "مش موجود عندي"
    #
    # التالتة دي هي اللي حسمت: مطاردة صيغ الرد لعبة الخسارة فيها مؤكدة،
    # لأن الصيغ مالهاش آخر واللي بيكتبها موديل مش قايمة ثابتة.
    #
    # الشرط الحقيقي مالوش علاقة بالصياغة أصلاً: **سؤال تكلفة عن مشروع
    # اترد عليه من غير ما المقايسة تتقرا**. أيًا كان اللي اتقال، الرد ده
    # مش مبني على المصدر الوحيد اللي فيه الرقم. الملاحظة بتقول الحقيقة دي
    # بالظبط -- وهي صحيحة سواء الرد نفى أو أكّد أو سأل سؤال.
    if cost_question and not no_turn_context and not read_estimate:
