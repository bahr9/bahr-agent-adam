...
# نفي وجود بيانات (مش نفي وجود أداة -- ده الحارس التاني)
_NO_DATA_PATTERNS = [
    r"مفيش أي", r"مفيش بند", r"مفيش بنود", r"مفيش تفاصيل", r"مفيش مقايس",
    r"مفيش تكلف", r"مش مسجل", r"مش متسجل", r"مالهوش بنود", r"لا يوجد أي",
    r"مفيش أسعار", r"مفيش داتا", r"مفيش بيانات",
]

# الكلام اللي بيحدد إن الموضوع مقايسات مشروع تحديدًا
_ESTIMATE_TOPIC_PATTERNS = [
    r"مقايس", r"بنود", r"تأسيس", r"تشطيب", r"فينيش", r"حصر", r"كميات",
]
...
def guard_against_unread_no_data_claims(reply_text: str, chat_id=None) -> str:
    """نفي وجود بيانات مقايسات من غير ما `get_project_details` تتنادى.
    ...
    بيرجع النص زي ما هو لو مفيش ادّعاء، أو النص + ملاحظة تصحيحية.
    """
    if not reply_text:
        return reply_text
    ...
    # --- (1) نفي بيانات مقايسات من غير قراءة ---
    denies_data = any(re.search(p, reply_text) for p in _NO_DATA_PATTERNS)
    about_estimates = any(re.search(p, reply_text) for p in _ESTIMATE_TOPIC_PATTERNS)
    if denies_data and about_estimates:
        # None = مفيش دليل خالص (سجل فاضي) -- مانتعاملش معاها كأنها "ماتنادتش"
        # عشان مانطلعش ملاحظة في كل رد على نظام لسه ماسجّلش حاجة.
        selected = _recently_selected("get_project_details", lookback=6)
        if selected is False:
            logger.warning(
                "⚠️ Contradiction prevented: الرد نفى وجود بيانات مقايسات و"
                "get_project_details ماتنادتش في آخر 6 اختيارات مسجّلة"
            )
            notes.append(
                "التفاصيل المالية للمشاريع (بنود المقايسات وأسعارها) موجودة في "
                "get_project_details، ودي **ماتنادتش** في الرد ده -- فنفي وجودها "
                "مش مبني على قراءة. الصح: 'مقراتش التفاصيل' مش 'مفيش تفاصيل'."
            )

    # --- (2) اختراع قاعدة عن قدرات BAHR OS ---
    invented = [p for p in _INVENTED_CAPABILITY_PATTERNS if re.search(p, reply_text)]
    if invented:
        logger.warning(
            "⚠️ Contradiction prevented: الرد اخترع قاعدة عن قدرات BAHR OS"
        )
        notes.append(
            "BAHR OS **بيسجّل** المقايسات فعلًا: مقايسات التأسيس والتشطيب "
            "والفينيش والديكور وحصر الكميات، ببنودها وأسعارها. أي كلام فوق "
            "بيقول إنه بيتابع الحالة بس مش دقيق ومايتبنيش."
        )
