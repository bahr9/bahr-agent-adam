...
comment on column public.projects.updated_at is
    'قرار معتمد 2026-08-05. المشكلة: الفرونت بيكتب Date.now() رقم وآدم بيكتب '
    'ISO نص، و Firestore بيرتّب كل الأرقام قبل كل النصوص -- فأي مشروع آدم لمسه '
    'بيترتّب بعد كل مشاريع الفرونت في orderBy(updatedAt desc) مهما كان أحدث، '
    'والتاريخ المعروض بيفضل صح فالباگ مابيعلنش عن نفسه. '
    'العمود ده timestamptz، فنوع واحد بس ممكن يتخزن فيه -- والاختلاط مستحيل '
    'بعد الهجرة مهما كان اللي بيكتب. التحويل وقت الاستيراد بيفرّع حسب نوع '
    'القيمة: رقم -> to_timestamp(v/1000)، نص -> ::timestamptz. '
    'وآدم كان بيكتب last_updated كمان -- نسخة تانية من نفس المعنى، بتتدمج هنا.';
...
create table public.invoices (
    ...
    -- علامة مراجعة -- تتحدد وقت الاستيراد. اقرا الملحوظة تحت الجدول.
...
-- ============================================================
-- 🔴 تحديد الفواتير المشكوك فيها -- تصحيح مهم
-- ============================================================
-- الخطة كانت: نعلّم الفواتير اللي اتعملت "قبل نشر v2".
--
-- **مفيش نشر لـ v2.** اتأكدت بالمقارنة المباشرة (2026-08-05):
--   - public/index.html (اللي firebase.json بيخدمه) لسه فيه
--     `JSON.parse(localStorage.getItem(key) || '{"items":[]}')` جوه getLaborData
--   - النسخة المصلَّحة اللي بتقرا من projectData موجودة في BAHR_OS_v2.html بس
--   - الملفين مختلفين في 514 سطر
--
-- يعني **كل فاتورة اتولدت لحد النهاردة عدّت على المسار الباگ**، والتعليم
-- بالتاريخ مالوش أي معنى -- هيعلّم كل حاجة، وعلامة على الكل = مفيش علامة.
--
-- البديل: تعليم **بالدليل** مش بالتاريخ.
--
-- الباگ إن getLaborData كان بيقرا أسطر الأتعاب من localStorage بتاع
-- المتصفح (مفاتيح bahrFoundationData/bahrFinishData/bahrFinalData)، واللي
-- فيه بيانات **آخر مشروع اتفتح في التبويب ده** -- مش بالضرورة المشروع
-- اللي بتتعمله فاتورة. فلو المستخدم فتح مشروع أ وبعدين عمل فاتورة
-- لمشروع ب، الفاتورة طلعت بأسطر مشروع أ.
--
-- والأثر ده **قابل للرصد**: أسماء أسطر الفاتورة مقارنة بأوصاف أسطر
-- تقدير نفس المشروع.
--
-- الفحص وقت الاستيراد، لكل فاتورة:
--   1. هات أوصاف أسطر التقدير للمشروع والمرحلة بتوعها
--      (لـ finalAll: اتحاد التلات مراحل)
--   2. قارنها بأسماء أسطر الفاتورة
--   3. التصنيف:
--        مفيش تطابق خالص والتقدير مش فاضي  -> needs_review = true
--                                              'items match another project'
--        التقدير فاضي والفاتورة فيها أسطر    -> needs_review = true
--                                              'no estimate to source from'
--        تطابق جزئي                          -> needs_review = true
--                                              'partial mismatch'
--        تطابق كامل                          -> false (المشروع الصح كان مفتوح)
--
-- ملحوظة: التطابق الكامل مايثبتش إن الفاتورة سليمة -- بس بيقول إن
-- مفيش أثر للباگ فيها. الفواتير المتعلّمة هي اللي عندنا **دليل** عليها.
--
-- سؤال لأحمد وقت التعليم (مش دلوقتي): الفواتير المتعلّمة تتشال من
-- التقارير المالية لحد المراجعة، ولا تتعرض ومعاها تحذير؟
...
-- ============================================================
-- قرارات معتمدة (2026-08-05)
-- ============================================================
-- ✅ updatedAt: العمود timestamptz، والتحويل بيفرّع حسب نوع القيمة وقت
--    الاستيراد. ده بيقفل اختلاط الأنواع نهائيًا بعد الهجرة مهما كان
--    اللي بيكتب. الإصلاح الجذري الصح -- إن الفرونت يكتب ISO زي آدم --
--    لسه مطلوب، بس بقى تحسين مش شرط للهجرة.
--
--    ⚠️ ملحوظة توقيت: طول ما الفرونت بيقرا من Firestore، ترتيب المشاريع
--    عنده هيفضل غلط. الإصلاح بيسري على Postgres من أول يوم، وعلى Bahr OS
--    لما يتحوّل هو كمان.
--
-- ✅ الفواتير المشكوك فيها: تتعلّم needs_review **وقت الاستيراد**، بالدليل
--    مش بالتاريخ (شوف الملحوظة الطويلة فوق جدول invoices). العمود بيفضل
--    فاضي لحد ساعتها.
--
-- ============================================================
-- قرارات لسه مفتوحة (مش مانعة إنشاء الجداول)
-- ============================================================
-- 1. projects.note (الفرونت) مقابل projects.adam_notes (آدم): ندمجهم
--    في عمود واحد ولا نسيبهم منفصلين؟ الدمج الصامت ممكن يضيّع الأقدم.
--
-- 2. مفتاح ثابت يربط سطر التقدير بعناصر PROJECT_ITEMS: بيصلح هشاشة
--    حقيقية، بس بيغيّر مخرجات الفواتير -- قرار شغل مش schema.
--
-- 3. الفواتير المتعلّمة: تتشال من التقارير المالية لحد المراجعة، ولا
--    تتعرض ومعاها تحذير؟ (القرار وقت التعليم، مش دلوقتي)
