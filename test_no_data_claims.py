...
# الجولة التالتة (2026-08-15) -- الرد ده عدّى من الحارس بسبب **حرف واحد**:
# "مش موجودة عندي" والنمط كان "مش موجود عندي". وأخطر منه إن سجل الدورة
# بيقول `selected_tools: []` -- ولا أداة اتنادت خالص. الرد نفسه بيقول
# "لسه نفس الإجابة"، يعني الموديل نسخ نفيه القديم من تاريخ المحادثة.
Q_ELEC2 = "تقدر تقولى قيمة تاسيس الكهرباء عند عصام فرج كام"
R_ELEC2 = (
    "للأسف لسه نفس الإجابة -- **قيمة تأسيس الكهرباء عند عصام فرج مش موجودة "
    "عندي في أي مكان**: مش في ملف المشروع، مش في المصاريف، ومش في BAHR OS.\n\n"
    "يعني أنا مش عارف الرقم ده، مش إن الرقم صفر -- ببساطة **محدش قاليه لي "
    "قبل كده** عشان أسجله."
)
TOOLS_ELEC2 = []          # ولا أداة
# الحقيقة: «تاسيس الكهرباء» 1 × 18,000.
...
def main():
    ...
    def a_reply_with_no_tool_call_at_all_is_caught():
        """أخطر شكل: الموديل رد من ذاكرة المحادثة، صفر أدوات.

        كل الإصلاحات اللي قبل كده -- مخرج الأداة، وصفها، قاعدة التوجيه --
        بتفترض إن الأداة هتتنادى. الرد ده ماناداش حاجة، وقال بنفسه "لسه
        نفس الإجابة" -- يعني نسخ نفيه القديم من التاريخ.

        وعدّى من الحارس بسبب تاء مربوطة واحدة: "مش موجودة عندي" ضد
        "مش موجود عندي". عشان كده الشرط دلوقتي مابيقراش صياغة النفي أصلاً.
        """
        out = guard(Q_ELEC2, R_ELEC2, TOOLS_ELEC2)
        assert out != R_ELEC2, "رد من غير أي نداء أداة عدّى"
        assert "get_project_details" in out, out

    def the_detector_survives_spelling_variants():
        """نفس السؤال بأربع صيغ إملائية -- كلهم لازم يتمسكوا.

        الفئة دي كسرت الفحص أربع مرات (ة/ه، ى/ي، أ/ا). التوحيد بيقفلها،
        والاختبار ده هو اللي بيمنعها ترجع."""
        from services.tool_lifecycle_diagnostics import detect_project_cost_question

        # ⚠️ الصيغ دي مختارة عشان **متعديش من غير التوحيد**. أول نسخة من
        # الاختبار ده كانت فاضية: كل الأمثلة كان فيها "مقايسة" أو "بنود"،
        # وهي بتمشي مع أو من غير توحيد -- فالتحوير على التوحيد طلع أخضر.
        # اللي تحت مفيهوش أي مفردة مقايسة، فالمرور بيعتمد على التوحيد بس.
        for q in ("أسعار الكهرباء عند عصام فرج",        # أ -> ا
                  "قيمه الدهانات بتاعة المشروع",        # ة -> ه
                  "إجمالي المصنعيات عند العميل"):       # إ -> ا
            assert detect_project_cost_question(q), "فات عليه: " + q

        # وصيغ فيها مفردة مقايسة -- بتعدي بأي حال، بس بتوثّق الاتساع
        for q in ("قيمة تأسيس الكهرباء فى مقايسة التاسيس",
                  "قيمه تاسيس الكهرباء في مقايسه التأسيس"):
            assert detect_project_cost_question(q), "فات عليه: " + q
    ...
    for name, fn in [
        ...
        ("رد من غير أي نداء أداة بيتمسك", a_reply_with_no_tool_call_at_all_is_caught),
        ("الكاشف بيصمد للاختلاف الإملائي", the_detector_survives_spelling_variants),
